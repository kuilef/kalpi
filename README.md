...
- структура из 12 тематических групп: обычно фундаментальные вопросы имеют вес `0.6`, policy-вопросы — `0.4`;
- ranking по 12 партиям: ближайшая партия, почти равные альтернативы в пределах 3 п.п., match, coverage и отставание от лидера;
- подробная трассировка по тематическим группам и вопросам с исходным status, confidence, entity scope и ссылками на evidence;
...
Для ответа пользователя `null` вопрос не участвует в компоненте и не увеличивает объём использованной информации. Ranking строится только после минимум восьми содержательных ответов в шести тематических группах; партия становится eligible при user-specific coverage не ниже 50%.
...
Текущий `scoring-config.json` содержит `"user_importance_enabled": true`. Пользователь может отметить отвеченный вопрос как важный: его вес в ranking и coverage становится `2` вместо `1`. Это влияет только на сопоставление с партиями, но не на тематические оси; правила и влияние на coverage опубликованы в методике.
...
- `data/scoring-config.json` — версии, шкала ответов, тематические группы, веса и release gate;
