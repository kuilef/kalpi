...
- 23 основных вопроса, показываемые по одному;
...
- подробная трассировка по тематическим группам и вопросам с исходным статусом, уровнем уверенности, охватом сущности и ссылками на подтверждающие материалы;
- публичная [страница аналитики](analytics.html) с проверкой готовности релиза, тепловой матрицей, фильтрами, карточкой ячейки и очередью перепроверки;
- резервный режим приложения: если проверка готовности релиза не проходит, рейтинг не показывается, даже когда в конфиге записан `live`.
...
`confidence` ограничивается диапазоном от `0` до `1`. В этой ветке прототипа действует явно опубликованная временная политика `all_value_positions_full_confidence`: любая содержательная позиция (`value` не `null`) получает для рейтинга расчётную уверенность `1`, независимо от исходных статуса и уверенности. Исходные поля не переписываются и видны в аналитике. `insufficient_data` остаётся отсутствующей позицией: механизм расчёта использует нейтральный вклад `0.5` и нулевое покрытие по этой ячейке.
...
Текущий `scoring-config.json` содержит `"user_importance_enabled": true`. Пользователь может отметить отвеченный вопрос как важный: его вес в рейтинге и покрытии становится `2` вместо `1`. Это влияет только на сопоставление с партиями, но не на тематические оси; правила и влияние на покрытие опубликованы в методике.
...
Поэтому позиция должна сопровождаться информацией о происхождении данных: источником, датой, цитатой или описанием подтверждений, уровнем уверенности и объяснением спорных случаев. Отсутствие надёжных данных не должно автоматически интерпретироваться как центристская позиция партии.
...
Поэтому важность для пользователя следует рассматривать как полезный сигнал, а не как абсолютную меру политической значимости. Более жёсткий вариант — попросить выбрать несколько тем, особенно важных для голоса, и отдельно проверить чувствительность результата к этим приоритетам.
...
Нет единственной математически правильной функции. Формула должна быть простой, опубликованной, воспроизводимой и проверяемой анализом чувствительности. Для текущей ветки формула опубликована выше, но полноценная проверка устойчивости рейтинга ещё не реализована — и сам рейтинг пока отключён.
