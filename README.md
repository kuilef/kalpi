...
- структура из 14 тематических групп: обычно фундаментальные вопросы имеют вес `0.6`, конкретные вопросы о политических мерах — `0.4`; две группы состоят только из конкретных вопросов и используют вес `1`;
...
В текущем наборе `immigration_identity` и `government_coalition` являются policy-only группами: в них нет фундаментального компонента, поэтому конкретные вопросы усредняются с весом `1`. Это не индивидуальный приоритет вопроса, а вес компонента внутри отдельной тематической группы.
