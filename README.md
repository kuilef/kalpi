...
Любая содержательная позиция (`value` не `null`) учитывается с полным весом. Служебное поле `confidence` в JSON не влияет на рейтинг, покрытие или release gate и не выводится в публичном интерфейсе. `insufficient_data` остаётся отсутствующей позицией: механизм расчёта использует нейтральный вклад `0.5` и нулевое покрытие по этой ячейке.
...
- `data/positions.json` — полная матрица `party × question`: 246 из 264 ячеек содержат позицию, 18 остаются `insufficient_data`;
