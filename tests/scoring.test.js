...
const config = {
  ...
  user_importance_family_multiplier: 2,
  families: [
    {
      id: 'family_a',
...
test('recommendation requires broad substantive answers and marks a near tie', () => {
  ...
  assert.deepEqual(recommendation.nearTies.map((item) => item.partyId), ['thin', 'second']);
...
test('an important question doubles its thematic family weight without changing local or coverage weights', () => {
  const result = Scoring.scoreParty({
    partyId: 'party',
    ...
    positions: [
      ...
      { party: 'party', question: 'b1', value: -1, confidence: 1, status: 'known' },
      { party: 'party', question: 'b2', value: 1, confidence: 1, status: 'known' },
      { party: 'party', question: 'a2', value: 1, confidence: 0, status: 'known' },
    ],
  ...
  assert.equal(family.policy.score, 0.5);
  assert.equal(family.score, 0.8);
  assert.equal(family.coverage, 1);
  assert.ok(Math.abs(result.score - 0.7) < 1e-12);
  assert.equal(result.coverage, 0.5);
});

test('marking two questions in one family does not stack the family multiplier', () => {
  const result = Scoring.scoreParty({
    partyId: 'party',
    answers: { a1: -1, b1: -1, b2: -1, a2: -1 },
    priorityQuestionIds: ['b1', 'b2'],
    positions: [
      { party: 'party', question: 'a1', value: -1, confidence: 1, status: 'known' },
      { party: 'party', question: 'b1', value: -1, confidence: 1, status: 'known' },
      { party: 'party', question: 'b2', value: 1, confidence: 1, status: 'known' },
...
test('unknown party position is excluded from score but reduces coverage', () => {
  const result = Scoring.scoreParty({
    partyId: 'party',
    answers: { a1: -1, b1: -1, b2: -1 },
    positions: [
  ...
  const family = result.families.find((item) => item.familyId === 'family_a');
  assert.equal(family.policy.score, 0);
  assert.equal(family.policy.coverage, 0.5);
  assert.equal(family.score, 0.6);
  assert.equal(family.coverage, 0.8);
  assert.equal(result.score, 0.6);
});

test('a component with no known party positions has no score but keeps zero coverage', () => {
  const result = Scoring.scoreParty({
    partyId: 'party',
    answers: { a1: -1 },
    positions: [],
  ...
  const family = result.families.find((item) => item.familyId === 'family_a');
  assert.equal(family.score, null);
  assert.equal(family.coverage, 0);
...
test('ranking treats floating-point equivalent scores as ties before coverage', () => {
  const tieConfig = {
    families: [
      { id: 'family_one', fundamental_questions: ['a1'], policy_questions: ['b1'], fundamental_weight: 0.6, policy_weight: 0.4, family_weight: 1 },
      { id: 'family_two', fundamental_questions: ['a2'], policy_questions: ['b2'], fundamental_weight: 0.6, policy_weight: 0.4, family_weight: 1 },
      { id: 'family_three', fundamental_questions: ['a3'], policy_questions: ['b3'], fundamental_weight: 0.6, policy_weight: 0.4, family_weight: 1 },
    ],
  };
  const answers = { a1: -1, b1: -1, a2: -1, b2: -1, a3: -1, b3: -1 };
  const positions = [
    { party: 'sparse', question: 'a1', value: -1, confidence: 1, status: 'known' },
    { party: 'sparse', question: 'b1', value: 0.5, confidence: 1, status: 'known' },
    ...['a1', 'b1', 'a2', 'b2', 'a3', 'b3'].map((question) => ({
      party: 'complete', question, value: question.startsWith('a') ? -1 : 0.5, confidence: 1, status: 'known',
    })),
  ];

  const ranked = Scoring.rankParties({
    parties: [{ id: 'sparse' }, { id: 'complete' }],
    answers,
    positions,
    scoringConfig: tieConfig,
  });

  assert.deepEqual(ranked.map((result) => result.partyId), ['complete', 'sparse']);
  assert.equal(ranked[1].score, 0.7);
  assert.ok(Math.abs(ranked[0].score - ranked[1].score) < 1e-12);
  assert.equal(ranked[0].coverage, 1);
  assert.equal(ranked[1].coverage, 1 / 3);
});
