...
HTTP smoke test прошёл. Прямое открытие через `file://` больше не поддерживается; приложение загружает canonical JSON через локальный или production HTTP-сервер.
