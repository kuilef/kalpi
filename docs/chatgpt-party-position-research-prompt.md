# Prompt для исследования одной партии

Этот текст нужно целиком вставить в новый чат ChatGPT. Затем загрузите перечисленные JSON-файлы и замените две переменные в начале: `TARGET_PARTY_ID` и `RESEARCH_DATE`.

На выходе будут два отдельных временных JSON-файла. Не копируйте их автоматически в репозиторий: сначала проведите ручную проверку, а затем перенесите подтверждённые изменения в canonical `data/sources.json` и `data/positions.json`.

## Что загрузить

1. `data/parties.json`
2. `data/questions.json`
3. `data/scoring-config.json`
4. `data/sources.json` — если хотите переиспользовать уже проверенные источники.
5. При повторном исследовании той же партии — её текущие строки из `data/positions.json`.
6. При необходимости — архивированные официальные программы, стенограммы или выгрузки голосований. Не загружайте старый `baseline-data.js` как доказательство.

Для первого запуска можно выбрать `TARGET_PARTY_ID: "likud"`. Новую партию исследуйте только в новом чате или после явной смены этой переменной.

## Текст для ChatGPT

````text
Ты готовишь временный исследовательский JSON-пакет для ручной проверки в Kalpi. Это не готовые canonical data и не партийная рекомендация. Твоя задача — по одной указанной партии последовательно оценить все 23 вопроса только по проверяемым источникам.

TARGET_PARTY_ID: "<замените на id из parties.json>"
RESEARCH_DATE: "<YYYY-MM-DD>"

Я загрузил файлы parties.json, questions.json и scoring-config.json. Возможно, также загружен sources.json и документы-источники.

Границы работы:

1. Исследуй только партию TARGET_PARTY_ID. Не меняй ни один вопрос, ID, полюс, тематическую группу (`family`), вес, версию или формулу scoring.
2. Верни ровно одну позицию для каждого из 23 core-вопросов из questions.json, в display_order. Никаких дополнительных вопросов.
3. Значение value относится только к полюсам конкретного вопроса:
   - -1: позиция прямо соответствует левому полюсу;
   - -0.5: скорее соответствует левому полюсу;
   - 0: есть прямые основания считать позицию действительно промежуточной или смешанной; это не «нет данных»;
   - 0.5: скорее соответствует правому полюсу;
   - 1: позиция прямо соответствует правому полюсу.
4. Не выводи value 0 как замену отсутствия информации. Если надёжной позиции нет, верни insufficient_data с value null и confidence 0.
5. Не выводи позицию по общему образу партии, по названию, по предположению или по мнению лидера без явной пометки scope. Если документ относится к компоненту списка, фракции, лидеру или прошлому периоду, это не автоматически позиция текущей партии.
6. Предпочитай прямые и актуальные источники: официальную программу, официальный сайт партии, официальное заявление, поимённое голосование, законопроект, стенограмму. Вторичный материал допустим только если в нём есть конкретное действие или цитата и ссылка доступна для проверки.
7. Не выдумывай URL, дату, цитату, название, source ID или политическую позицию. Если у тебя нет браузинга или загруженного подтверждения, это insufficient_data.
8. Для known, mixed или historical обязательны: value, confidence больше 0, хотя бы один evidence ID, короткое нейтральное explanation_ru и last_verified = RESEARCH_DATE.
9. confidence — это качество именно этой связи «источник → позиция», а не уверенность в твоём рассуждении:
   - 0.85–1.00: прямой, текущий и однозначный материал партии или списка;
   - 0.60–0.80: конкретное голосование, заявление лидера/фракции или хороший, но не полностью прямой материал;
   - 0.25–0.55: историческое, компонентное или противоречивое свидетельство;
   - 0: только insufficient_data.
10. Используй status:
    - known — текущая достаточно ясная позиция;
    - mixed — есть существенные противоречия внутри партии или между прямыми материалами; объясни их;
    - historical — материал прямой, но относится к прошлому периоду и не подтверждает текущую линию;
    - insufficient_data — позиции нельзя установить без домысла.
11. Используй entity_scope только из этого набора: PARTY, CURRENT_LIST, FACTION, LEADER, COMPONENT_PARTY, HISTORICAL. Уточняй scope по фактическому субъекту источника.
12. Если source ID уже есть в загруженном sources.json, используй его без изменений. Для нового источника добавь объект в массив sources с уникальным ID вида candidate_<party>_<короткое_имя>. Эти источники остаются candidate_unverified и не считаются проверенными автоматически.
13. Для insufficient_data верни строго:
    {"value": null, "confidence": 0, "status": "insufficient_data", "entity_scope": "PARTY", "evidence": [], "explanation_ru": "", "last_verified": null}
14. Создай ровно два файла. Если интерфейс умеет создавать вложения, приложи два JSON-файла с указанными ниже путями. Если вложения недоступны, выведи ровно два подписанных блока ```json: в каждом блоке должен быть только JSON, без комментариев и текста внутри.
15. Не создавай и не перезаписывай data/sources.json, data/positions.json, scoring, ranking, mapping тематических групп (`family`) или файлы проекта.

Первый файл: sources.json

```json
[
  {
    "id": "candidate_<party>_<короткое_имя>",
    "title": "Название источника в оригинале или точном переводе",
    "url": "https://...",
    "source_type": "party_platform | official_statement | parliamentary_vote | bill | transcript | reputable_reporting | other",
    "date": "YYYY-MM-DD или null",
    "notes_ru": "Что именно источник подтверждает и к какому субъекту относится.",
    "verification_status": "candidate_unverified"
  }
]
```

Этот массив содержит только новые candidate sources. Если новых источников нет, верни `[]`. Уже существующие IDs из загруженного `sources.json` сюда не копируй.

Второй файл: positions.json

```json
[
  {
    "party": "<TARGET_PARTY_ID>",
    "question": "<ровно один id из questions.json>",
    "value": -1,
    "confidence": 0.9,
    "status": "known",
    "entity_scope": "PARTY",
    "evidence": ["существующий_или_candidate_source_id"],
    "explanation_ru": "Короткое нейтральное объяснение: что именно делает или заявляет партия и почему это соответствует этому полюсу вопроса.",
    "last_verified": "<RESEARCH_DATE>"
  }
]
```

В `positions.json` должно быть ровно 23 записи без повторов: по одной для каждого core-вопроса в display_order. Перед выводом проверь: party совпадает с TARGET_PARTY_ID; каждый question существует; все evidence IDs существуют либо в загруженном `sources.json`, либо в первом output-файле; insufficient_data не имеет источников и не превращается в value 0.
````

## Что делать с ответом

Сохраните оба файла ответа вне рабочей ветки до завершения ручной проверки. Не заменяйте ими canonical-файлы автоматически.

```text
<temporary-folder>/likud/sources.json
<temporary-folder>/likud/positions.json
```

Сначала вручную проверьте URL, дату, цитату или голосование, субъект позиции и направление относительно полюсов вопроса. Затем отдельным шагом добавьте подтверждённые источники в `data/sources.json` и замените только соответствующие строки целевой партии в `data/positions.json`. Не подтверждённые строки остаются `insufficient_data`.

После каждой партии запускайте:

```powershell
& "C:\Users\User\.cache\codex-runtimes\codex-primary-runtime\dependencies\node\bin\node.exe" --test tests/*.test.js
```

`recommendation_mode` остаётся `data_not_ready`, пока отдельно не согласованы evidence review и coverage gate.
