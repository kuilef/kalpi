...
Kalpi (קלפי, «избирательная урна») — политический опросник для сопоставления своих взглядов с позициями партий на выборах в Кнессет 2026. Сайт включает [опросник](index.html), [описание методологии](methodology.html) и [аналитику данных](analytics.html).

Интерфейс доступен на русском, английском и иврите. Для иврита используется направление письма справа налево (RTL). Числа и описание реализации ниже сверены с файлами репозитория 6 сентября 2026 года.

## Что работает сейчас

- 26 основных вопросов, показываемых по одному;
...
- сопоставление с 12 партиями; в результатах показываются до семи партий, прошедших порог покрытия, их совпадение, покрытие данных и отставание от лидера; если первые три подходящие партии (или две, когда доступны только две) укладываются в 5 п.п., появляется пояснение о близких результатах;
...
Текущий `scoring-config.json` содержит `"user_importance_enabled": true` и множитель тематической группы `"user_importance_family_multiplier": 2`. Пользователь может отметить вопрос с содержательным ответом как важный: базовый `family_weight` всей его тематической группы умножается на `2` в итоговом сопоставлении. Вес `1` становится `2`, а вес `0.5` — `1`. Несколько важных вопросов одной группы не складываются. Покрытие данных и результаты внутри тематических групп не зависят от важности.
...
Невозможно включить в короткий опросник все политические вопросы. При простом усреднении всех вопросов область с большим числом вопросов получает больше влияния на итоговое совпадение. Тематические группы Kalpi уменьшают этот эффект, но их состав, базовые веса и выбор вопросов внутри каждой группы всё равно влияют на результат.

Поэтому при пересмотре анкеты важно проверять тематическое покрытие, баланс между основными политическими областями, отсутствие почти одинаковых вопросов и способность каждого вопроса различать реальные партии. См. König & Nyhuis, [*Assessing the applicability of vote advice applications for estimating party positions*](https://doi.org/10.1177/1354068818790111), а также Garzia & Marschall, [*Research on Voting Advice Applications: State of the Art and Future Directions*](https://doi.org/10.1002/poi3.140).
...
Результат показывает совпадение и покрытие данных партии именно для содержательных ответов пользователя. `coverage` — взвешенная доля доступных партийных позиций с учётом компонентов и тематических групп, а не доля отвеченных вопросов во всей анкете. Оно рассчитывается с базовыми весами, без множителя важности. Даже высокое покрытие при небольшом числе ответов не означает, что сравнение охватывает все взгляды пользователя; поэтому отдельно действуют пороги числа ответов и тематических групп.
...
Нет единственной математически правильной функции. Формула должна быть простой, опубликованной, воспроизводимой и проверяемой анализом чувствительности. Используемая формула опубликована выше.
...
Для проверки этих ограничений в репозитории доступны:
...
- формулу расчёта и правила обработки ответа `null` и статуса `insufficient_data`;
...
Русские формулировки хранятся в исходных HTML/JS-файлах и канонических данных. Английский и ивритский переводы находятся в `locales/`:

- `ui-en.json` / `ui-he.json` — общий интерфейс;
- `app-en.json` / `app-he.json` — строки опросника и результатов;
- `pages-en.json` / `pages-he.json` — тексты страниц;
- `data-en.json` / `data-he.json` — переводы текстов из данных.

Переключение языка сохраняет ответы пользователя; позиции партий, формулы и веса общие для всех языков.

Документация доступна в [README.md](README.md) на английском, [README.ru.md](README.ru.md) на русском и [README.he.md](README.he.md) на иврите. При изменении поведения или методологии обновляйте все три версии. Идентификаторы, команды, пути и формулы не переводятся.
...
В конфигурации `near_tie_points: 0.03` задаёт вычисляемый признак близости к лидеру в пределах 3 п.п.; пояснение о близости первых мест в интерфейсе использует отдельный порог 5 п.п.
...
Для локального запуска нужен Python 3; для JavaScript-проверок — Node.js.
